 ...
 * 解题思路：如果想找到一个相交节点，如果从前面开始进行比较的话，我们则需要往后开始，逐一进行处理，这样的话，复杂度会很高。
 * 既然是相交节点，说明后面的节点其实是一样的，我们可以考虑从后面开始来往前遍历
 * 那么，这时候我们就需要考虑，如何能够从后往前呢？使用栈全部保存起来即可
 * 时间复杂度：O（N）：我们需要将数据进行两次的遍历，并保存下来
 * 空间复杂度：O（N）：需要用栈来保存
 */
class SolutionLT160 {
    public static void main(String[] args) {
        SolutionLT160 lt = new SolutionLT160();

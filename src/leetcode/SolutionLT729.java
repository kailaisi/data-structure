...
class SolutionLT729 {
    ...
    public static void main(String[] args) {
        SolutionLT729 lt = new SolutionLT729();
    ...
    public boolean book(int start, int end) {
        for (int[] ints : list) {
            int s1 = ints[0];
            int e1 = ints[1];
            if(start>=s1 && start<e1){//start位于s1和e1之间
                return false;
            }else if(end>s1 && end<e1){//end位于s1和e1之间
                return false;
            }
            else if(s1>=start && s1<end){//s1位于start和end之间
                return false;
            }else if(e1>start && e1<end){//e1位于start和end之间
                return false;
            }
        }
